...
function getArticleUrl(site: URL | undefined, post: BlogEntry): string {
  ...
  return new URL(`/${post.data.lang}/article/${post.data.i18nKey}/`, base).toString();
}

function getPdfFileName(post: BlogEntry): string {
  const lastSegment = post.data.i18nKey.split("/").at(-1) || "article";
  return `${post.data.lang}-${lastSegment.replace(/[^\p{L}\p{N}._-]+/gu, "-")}.pdf`;
}
...
function writeGeneratedArticleSource(projectRoot: string, post: BlogEntry, sourceUrl: string): string {
  const generatedDir = path.join(projectRoot, ".astro", "generated-article-pdfs", post.data.lang);
  mkdirSync(generatedDir, { recursive: true });

  const generatedFileName = `${post.data.i18nKey.replace(/[^a-zA-Z0-9._-]+/g, "__")}.typ`;
...
export async function GET({ props, params, site }: APIContext<{ post: BlogEntry }>) {
  ...
  if (!post) {
    throw new Error(`[article-pdf] Missing post props for ${params.locale}/${params.slug}`);
  }
  ...
  try {
    ...
    console.error(`[article-pdf] Failed to compile ${post.data.lang}/${post.data.i18nKey}: ${e?.code ?? e?.message ?? String(e)}`);
    throw e;
  }
