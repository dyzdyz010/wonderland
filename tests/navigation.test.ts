...
describe("navigation active state", () => {
  test("normalizes trailing slashes, query strings, hashes, and leading slashes", () => {
    ...
    expect(normalizeNavigationPath("en/")).toBe("/en");
  ...
  test("treats the unprefixed root as the default locale home", () => {
    expect(isNavigationLinkActive("/en/", "/", "exact")).toBe(true);
    expect(isNavigationLinkActive("/zh/", "/", "exact")).toBe(false);
  });
